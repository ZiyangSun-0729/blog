...
master01   NotReady   <none>   3m58s   v1.32.0
master02   NotReady   <none>   39s     v1.32.0
master03   NotReady   <none>   41s     v1.32.0
node01     NotReady   <none>   43s     v1.32.0
node02     NotReady   <none>   34s     v1.32.0
```
...
```
 ...
 为 `calico-kube-controllers` 创建 `ClusterRoleBinding`

~~~~
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRoleBinding
metadata:
  name: calico-kube-controllers-binding
subjects:
  - kind: ServiceAccount
...
~~~~
kubectl apply -f calico-kube-controllers-binding.yaml
~~~~
...
===========================================================
NAME       STATUS   ROLES    AGE   VERSION
master01   Ready    <none>   19m   v1.32.0
master02   Ready    <none>   15m   v1.32.0
master03   Ready    <none>   15m   v1.32.0
node01     Ready    <none>   15m   v1.32.0
node02     Ready    <none>   15m   v1.32.0
```
...
-  资源监控：Metrics Server会周期性地收集每个节点和Pod的CPU、内存使用情况，并暂时存储在内存中。 

-  自动扩缩：HPA根据Metrics Server提供的CPU利用率来决定是否需要增加或减少Pod的副本数，对于负载波动较大的应用非常有用。  
...
在文件中添加`--kubelet-insecure-tls`参数，忽略证书检测  ，默认情况下Metrics Server会使用TLS（加密方式）与Kubelet通信， 而Kubelet使用的是自签名证书，Metrics Server无法验证，进而导致通信失败。
...
使用现有的CA证书及其私钥 ，对`Metrics Server`签名请求文件进行签署，生成一个新的https证书和对应的私钥
...
将Metrics Server证书注册到`API Server`中，需要在`kube-apiserver.conf`文件增加以下内容，具体添加方法，参考下边Master01节点
...
<details class="lake-collapse"><summary id="u93d4ff72"><strong><span class="ne-text">参数说明：</span></strong></summary><p id="u60dfd28a" class="ne-p" style="margin: 0; padding: 0; min-height: 24px"><span class="ne-text">runtime-config=api/all=true		</span><span class="ne-text" style="color: #DF2A3F">//启用所有API版本和资源，主要支持资源监控功能。</span></p><p id="u6201277a" class="ne-p" style="margin: 0; padding: 0; min-height: 24px"><span class="ne-text">requestheader-allowed-names	</span><span class="ne-text" style="color: #DF2A3F">//允许"aggregator"用户向API发送请求，用户在证书中已定义。</span></p><p id="u86b95a99" class="ne-p" style="margin: 0; padding: 0; min-height: 24px"><span class="ne-text">requestheader-group-headers	</span><span class="ne-text" style="color: #DF2A3F">//用户所在的组信息将通过X-Remote-Group请求头告诉API。</span></p><p id="ubdcf76ae" class="ne-p" style="margin: 0; padding: 0; min-height: 24px"><span class="ne-text">requestheader-username-headers		</span><span class="ne-text" style="color: #DF2A3F">//用户名将通过X-Remote-User这个请求头告诉API。</span></p><p id="u566fb517" class="ne-p" style="margin: 0; padding: 0; min-height: 24px"><span class="ne-text">requestheader-extra-headers-prefix=X-Remote-Extra-		</span><span class="ne-text" style="color: #DF2A3F">//请求头中需要检查的前缀名。</span></p></details>

Master01节点添加方法，添加到文件末尾
...
其他Master节点也要添加，添加完成后，三台节点重启API server服务
...
查看Metrisc Server的Pod

```shell
kubectl get pod -n kube-system
```

Metrisc Server运行后，通过`kubectl top`查看监控数据

**提示：**`kubectl top` 命令需要Metrisc Server运行后才能用
...
**案例**：通过HPA控制器帮助DeployMent创建Pod，并根据Pod的CPU负载量对Pod数量自动扩缩容。